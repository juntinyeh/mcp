...
async def _update_results(main_results: Dict[str, Any], service_results: Dict[str, Any], service_name: str, include_unencrypted_only: bool) -> None:
...
async def generate_recommendations(results: Dict[str, Any]) -> List[str]:
    ...
    recommendations.append("Use customer-managed KMS keys instead of AWS managed keys for sensitive data")
    recommendations.append("Implement a key rotation policy for all customer-managed KMS keys")
    
    return recommendations

async def find_storage_resources(region: str, session: boto3.Session, services: List[str], ctx: Context) -> Dict[str, Any]:
    """Find storage resources using Resource Explorer."""
    try:
        print(f"[DEBUG:StorageSecurity] Finding storage resources in {region} using Resource Explorer")
...
async def check_s3_buckets(region: str, s3_client: boto3.client, ctx: Context, storage_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'error' not in storage_resources and 's3' in storage_resources.get('resources_by_service', {}):
            ...
            for bucket in response['Buckets']:
                ...
                try:
                    ...
                    print(f"[DEBUG:StorageSecurity] Error getting location for bucket {bucket['Name']}: {e}")
        ...
        for bucket_name in buckets:
            ...
            try:
                ...
                if encryption_rules:
                    encryption_type = encryption_rules[0].get('ApplyServerSideEncryptionByDefault', {}).get('SSEAlgorithm')
                    ...
                    # Check if using CMK
                    kms_key = encryption_rules[0].get('ApplyServerSideEncryptionByDefault', {}).get('KMSMasterKeyID')
            ...
            try:
                ...
                print(f"[DEBUG:StorageSecurity] Error checking public access block for {bucket_name}: {e}")
                bucket_result['checks']['block_public_access'] = {
                    'enabled': False,
                    'error': str(e)
                }
            ...
            if not bucket_result['checks'].get('default_encryption', {}).get('enabled', False):
                bucket_result['remediation'].append("Enable default encryption using SSE-KMS or SSE-S3")
            
            if not bucket_result['checks'].get('block_public_access', {}).get('enabled', False):
                bucket_result['remediation'].append("Enable block public access settings for this bucket")
