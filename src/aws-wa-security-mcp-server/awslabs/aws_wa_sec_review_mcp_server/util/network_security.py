...
async def check_network_security(
    ...
    if 'elb' in services:
        ...
        elb_results = await check_classic_load_balancers(region, elb_client, ctx, network_resources)
        ...
        elbv2_results = await check_elbv2_load_balancers(region, elbv2_client, ctx, network_resources)
...
async def _update_results(main_results: Dict[str, Any], service_results: Dict[str, Any], service_name: str, include_non_compliant_only: bool) -> None:
...
async def generate_recommendations(results: Dict[str, Any]) -> List[str]:
    ...
    if 'elb' in results.get('compliance_by_service', {}) or 'elbv2' in results.get('compliance_by_service', {}):
        elb_non_compliant = results.get('compliance_by_service', {}).get('elb', {}).get('non_compliant_resources', 0)
        elbv2_non_compliant = results.get('compliance_by_service', {}).get('elbv2', {}).get('non_compliant_resources', 0)
        ...
        if elb_non_compliant > 0 or elbv2_non_compliant > 0:
            ...
            recommendations.append("Use AWS Certificate Manager (ACM) to provision and manage certificates")
    
    # Check VPC endpoint recommendations
    if 'vpc' in results.get('compliance_by_service', {}) and results.get('compliance_by_service', {}).get('vpc', {}).get('non_compliant_resources', 0) > 0:
        recommendations.append("Configure interface VPC endpoints to use TLS for all communications")
        recommendations.append("Enable private DNS for interface endpoints where applicable")
    
    # Check security group recommendations
    if 'security_groups' in results.get('compliance_by_service', {}) and results.get('compliance_by_service', {}).get('security_groups', {}).get('non_compliant_resources', 0) > 0:
    ...
    if 'apigateway' in results.get('compliance_by_service', {}) and results.get('compliance_by_service', {}).get('apigateway', {}).get('non_compliant_resources', 0) > 0:
    ...
    if 'cloudfront' in results.get('compliance_by_service', {}) and results.get('compliance_by_service', {}).get('cloudfront', {}).get('non_compliant_resources', 0) > 0:
        ...
        recommendations.append("Use Origin Access Identity (OAI) or Origin Access Control (OAC) for S3 origins")
...
async def find_network_resources(region: str, session: boto3.Session, services: List[str], ctx: Context) -> Dict[str, Any]:
    """Find network resources using Resource Explorer."""
    try:
        print(f"[DEBUG:NetworkSecurity] Finding network resources in {region} using Resource Explorer")
        ...
        if not default_view:
            ...
            await ctx.warning("No default Resource Explorer view found. Will fall back to direct service API calls.")
...
async def check_classic_load_balancers(region: str, elb_client: Any, ctx: Context, network_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'error' not in network_resources and 'elb' in network_resources.get('resources_by_service', {}):
        ...
        print(f"[DEBUG:NetworkSecurity] Found {len(load_balancers)} Classic Load Balancers in region {region}")
        ...
        for lb_name in load_balancers:
            ...
            lb_details = elb_client.describe_load_balancers(LoadBalancerNames=[lb_name])['LoadBalancerDescriptions'][0]
            
            # Check for HTTPS listeners
            listeners = lb_details.get('ListenerDescriptions', [])
            https_listeners = [l for l in listeners if l['Listener'].get('Protocol') in ['HTTPS', 'SSL']]
            http_listeners = [l for l in listeners if l['Listener'].get('Protocol') in ['HTTP', 'TCP']]
            ...
            for https_listener in https_listeners:
                ...
                try:
                    policy_response = elb_client.describe_load_balancer_policies(
                        ...
                        PolicyNames=[https_listener['PolicyNames'][0]] if https_listener.get('PolicyNames') else []
                    )
                    ...
                    if policy_response.get('PolicyDescriptions'):
                        ...
                        protocol_policies = [attr for attr in ssl_policy.get('PolicyAttributeDescriptions', []) 
                                           if attr['AttributeName'].startswith('Protocol-') and attr['AttributeValue'] == 'true']
                        
                        insecure_protocols = [p['AttributeName'].replace('Protocol-', '') 
                                            for p in protocol_policies 
                                            if p['AttributeName'].replace('Protocol-', '') in INSECURE_PROTOCOLS]
                        
                        if insecure_protocols:
                            lb_result['compliant'] = False
                            lb_result['issues'].append(f"Using insecure protocols: {', '.join(insecure_protocols)}")
            ...
            if lb_result['checks'].get('ssl_policies') and any(p.get('insecure_protocols') for p in lb_result['checks'].get('ssl_policies', [])):
...
async def check_elbv2_load_balancers(region: str, elbv2_client: Any, ctx: Context, network_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[DEBUG:NetworkSecurity] Found {len(load_balancers)} ALB/NLB Load Balancers in region {region}")
        results['resources_checked'] = len(load_balancers)
        
        # Check each load balancer
        for lb_arn in load_balancers:
            lb_name = lb_arn.split('/')[-2] if '/app/' in lb_arn or '/net/' in lb_arn else lb_arn.split('/')[-1]
            lb_type = 'application' if '/app/' in lb_arn else 'network' if '/net/' in lb_arn else 'unknown'
            ...
            try:
                ...
                if lb_type == 'application':
                    ...
                    if not all_secure:
                        lb_result['compliant'] = False
                        lb_result['issues'].append(f"Found {len(http_listeners)} non-encrypted HTTP listeners")
                    ...
                    for https_listener in https_listeners:
                        ...
                        if ssl_policy and not ssl_policy.startswith('ELBSecurityPolicy-TLS-1-2') and not ssl_policy.startswith('ELBSecurityPolicy-FS-1-2'):
                            lb_result['compliant'] = False
                            lb_result['issues'].append(f"Using potentially insecure SSL policy: {ssl_policy}")
            ...
            if lb_type == 'application' and not lb_result['checks'].get('all_listeners_secure', True):
                lb_result['remediation'].append("Replace HTTP listeners with HTTPS listeners")
                lb_result['remediation'].append("Configure HTTP to HTTPS redirection")
            
            if lb_result['checks'].get('ssl_policies'):
                lb_result['remediation'].append("Update SSL policy to ELBSecurityPolicy-TLS-1-2-2017-01 or newer")
...
async def check_vpc_endpoints(region: str, ec2_client: Any, ctx: Context, network_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'error' not in network_resources and 'vpc_endpoints' in network_resources.get('resources_by_service', {}):
            # Use Resource Explorer results
            ...
            vpc_endpoints = [endpoint['VpcEndpointId'] for endpoint in response.get('VpcEndpoints', [])]
        
        print(f"[DEBUG:NetworkSecurity] Found {len(vpc_endpoints)} VPC endpoints in region {region}")
        ...
        for endpoint_id in vpc_endpoints:
            ...
            if endpoint_type == 'Interface':
                ...
                if not private_dns_enabled:
                    ...
                    endpoint_result['issues'].append("Private DNS not enabled for interface endpoint")
            ...
            if endpoint_type == 'Interface' and 'Groups' in endpoint:
                ...
                endpoint_result['checks']['security_groups'] = [sg['GroupId'] for sg in security_groups]
...
async def check_security_groups(region: str, ec2_client: Any, ctx: Context, network_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'error' not in network_resources and 'security_groups' in network_resources.get('resources_by_service', {}):
            # Use Resource Explorer results
        ...
        print(f"[DEBUG:NetworkSecurity] Found {len(security_groups)} security groups in region {region}")
        ...
        for sg_id in security_groups:
            ...
            if open_sensitive_ports:
                ...
                for port_info in open_sensitive_ports:
                    sg_result['issues'].append(f"Port {port_info['port']} ({port_info['service']}) open to the world")
            
            # Generate remediation steps
            sg_result['remediation'] = []
            
            if open_sensitive_ports:
                sg_result['remediation'].append("Restrict access to sensitive ports to specific IP ranges")
                sg_result['remediation'].append("Replace insecure protocols with secure alternatives (e.g., HTTPS instead of HTTP)")
                sg_result['remediation'].append("Use security group source references instead of CIDR blocks where possible")
...
async def check_api_gateway(region: str, apigw_client: Any, ctx: Context, network_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'error' not in network_resources and 'apigateway' in network_resources.get('resources_by_service', {}):
            # Use Resource Explorer results
        ...
        for api_id in apis:
                    ...
                    for mapping in mappings:
                        if mapping.get('restApiId') == api_id:
                            ...
                            if security_policy != 'TLS_1_2':
                                api_result['compliant'] = False
                                api_result['issues'].append(f"Domain {domain_name} using insecure TLS policy: {security_policy}")
            ...
            if 'stages' in api_result['checks'] and any(not stage.get('https_enforced', False) for stage in api_result['checks'].get('stages', [])):
                api_result['remediation'].append("Enable 'Require HTTPS' in method settings")
            
            if 'domains' in api_result['checks'] and any(domain.get('security_policy') != 'TLS_1_2' for domain in api_result['checks'].get('domains', [])):
...
async def check_cloudfront_distributions(region: str, cf_client: Any, ctx: Context, network_resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'error' not in network_resources and 'cloudfront' in network_resources.get('resources_by_service', {}):
            # Use Resource Explorer results
        ...
        for dist_id in distributions:
            ...
            if viewer_protocol_policy != 'redirect-to-https' and viewer_protocol_policy != 'https-only':
                ...
                dist_result['issues'].append(f"Viewer protocol policy not enforcing HTTPS: {viewer_protocol_policy}")
            
            # Check TLS version
            ssl_protocol_version = config.get('ViewerCertificate', {}).get('MinimumProtocolVersion')
            dist_result['checks']['minimum_tls_version'] = ssl_protocol_version
            
            if ssl_protocol_version and ssl_protocol_version not in ['TLSv1.2_2018', 'TLSv1.2_2019', 'TLSv1.2_2021']:
                dist_result['compliant'] = False
            ...
            for origin in origins:
                if 's3' in origin.get('DomainName', '').lower():
                    ...
                    has_oai = 'S3OriginConfig' in origin and 'OriginAccessIdentity' in origin['S3OriginConfig'] and origin['S3OriginConfig']['OriginAccessIdentity']
            ...
            if s3_origins_without_oai:
                dist_result['compliant'] = False
                dist_result['issues'].append(f"S3 origins without OAI/OAC: {', '.join(s3_origins_without_oai)}")
            ...
            if viewer_protocol_policy != 'redirect-to-https' and viewer_protocol_policy != 'https-only':
                dist_result['remediation'].append("Set ViewerProtocolPolicy to 'redirect-to-https' or 'https-only'")
            
            if ssl_protocol_version and ssl_protocol_version not in ['TLSv1.2_2018', 'TLSv1.2_2019', 'TLSv1.2_2021']:
                dist_result['remediation'].append("Update MinimumProtocolVersion to TLSv1.2_2021")
            
            if s3_origins_without_oai:
                dist_result['remediation'].append("Configure Origin Access Identity (OAI) or Origin Access Control (OAC) for S3 origins")
