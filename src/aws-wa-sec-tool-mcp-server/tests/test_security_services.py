...
async def test_get_guardduty_findings_success(mock_ctx, mock_boto3_session, mock_guardduty_client):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.util.security_services.check_guard_duty"
    ) as mock_check:
...
async def test_get_securityhub_findings_success(
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.util.security_services.check_security_hub"
    ) as mock_check:
...
async def test_get_inspector_findings_success(mock_ctx, mock_boto3_session, mock_inspector_client):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.util.security_services.check_inspector"
    ) as mock_check:
        mock_check.return_value = {"enabled": True}
...
async def test_get_access_analyzer_findings_success(
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.util.security_services.check_access_analyzer"
    ) as mock_check:
        mock_check.return_value = {
            "enabled": True,
            "analyzers": [
...
async def test_get_trusted_advisor_findings_success(
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.util.security_services.check_trusted_advisor"
    ) as mock_check:
...
async def test_get_macie_findings_success(mock_ctx, mock_boto3_session, mock_macie_client):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.util.security_services.check_macie"
    ) as mock_check:
        mock_check.return_value = {"enabled": True}
