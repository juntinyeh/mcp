...
async def test_check_security_services(mock_ctx, mock_boto3_session):
    ...
    with (
        mock.patch(
            "awslabs.aws_wa_sec_tool_mcp_server.server.check_guard_duty"
        ) as mock_guard_duty,
        mock.patch("awslabs.aws_wa_sec_tool_mcp_server.server.check_inspector") as mock_inspector,
        mock.patch(
            "awslabs.aws_wa_sec_tool_mcp_server.server.check_access_analyzer"
        ) as mock_access_analyzer,
        mock.patch(
            "awslabs.aws_wa_sec_tool_mcp_server.server.check_security_hub"
        ) as mock_security_hub,
        mock.patch(
            "awslabs.aws_wa_sec_tool_mcp_server.server.check_trusted_advisor"
        ) as mock_trusted_advisor,
        mock.patch("awslabs.aws_wa_sec_tool_mcp_server.server.check_macie") as mock_macie,
    ):
...
async def test_get_security_findings_guardduty(mock_ctx):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.server.get_security_findings"
...
async def test_check_storage_encryption_tool(mock_ctx, mock_boto3_session):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.server.check_storage_encryption"
    ) as mock_check:
        # Set up the mock to return a specific value
        mock_result = {
            "region": "us-east-1",
...
async def test_list_services_in_region_tool(mock_ctx, mock_boto3_session):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.server.list_services_in_region"
    ) as mock_list:
        # Set up the mock to return a specific value
        mock_result = {
            "region": "us-east-1",
...
async def test_check_network_security_tool(mock_ctx, mock_boto3_session):
    ...
    with mock.patch(
        "awslabs.aws_wa_sec_tool_mcp_server.server.check_network_security"
    ) as mock_check:
        # Set up the mock to return a specific value
        mock_result = {
            "region": "us-east-1",
...
def test_main():
    ...
    with (
        ...
        mock.patch("awslabs.aws_wa_sec_tool_mcp_server.server.mcp") as mock_mcp,
    ):
        # Set up mock parser
